  ...
  ({ hintNumber, hintText, guess, targetYear, shouldReduceMotion = false }) => {
    ...
    return (
      <motion.div
        ...
        className={`rounded-lg border-2 px-4 py-3 ${backgroundClass} shadow-primary/5 hover:shadow-primary/10 opacity-90 shadow-md transition-all duration-150 hover:opacity-100 hover:shadow-lg`}
      >
  ...
  ({ hintNumber, hintText, shouldReduceMotion = false }) => {
    return (
      <motion.div
        ...
        className="border-muted/50 bg-muted/10 shadow-muted/10 hover:shadow-muted/20 rounded-lg border border-dashed px-4 py-3 opacity-75 shadow-sm transition-all duration-150 hover:opacity-90 hover:shadow-md"
      >
        ...
        <LayoutGroup>
          ...
          <AnimatePresence>
            {pastHints.map((hint, index) => (
              <motion.div key={`past-hint-${hint.hintNumber}`} layout={!shouldReduceMotion}>
                <PastHint
