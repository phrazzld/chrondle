...
export const CURATED_HISTORICAL_YEARS = [
  ...
  1620, 1642, 1648, 1666, 1682, 1688, 1707, 1721, 1740, 1751, 1756, 1769, 1773, 1776, 1789, 1804,
  1815, 1821, 1825, 1833,

  // Industrial Era (18 years) - Industrial revolution, nationalism, imperialism
  1837, 1845, 1848, 1859, 1861, 1865, 1869, 1871, 1876, 1885, 1886, 1896, 1898, 1899, 1900, 1903,
  1905, 1911,
...
export const CURATED_YEARS_META = {
  ...
  geographic_coverage: ["Europe", "Asia", "Americas", "Africa", "Middle East", "Oceania"],
  event_categories: [
    "Wars & Conflicts",
...
export function validateCuratedYears(): boolean {
  ...
  if (CURATED_HISTORICAL_YEARS.length !== 100) {
    logger.error(`Expected 100 years, got ${CURATED_HISTORICAL_YEARS.length}`);
    return false;
  }
  ...
  if (uniqueYears.size !== CURATED_HISTORICAL_YEARS.length) {
    logger.error("Duplicate years found in curated list");
    return false;
  }
  ...
  for (let i = 0; i < CURATED_HISTORICAL_YEARS.length; i++) {
    if (CURATED_HISTORICAL_YEARS[i] !== sorted[i]) {
      logger.error("Years are not in chronological order");
      return false;
    }
  }

  // Check that all current puzzle years are preserved
  const currentPuzzleYears = [
    1066, 1215, 1492, 1588, 1776, 1789, 1865, 1914, 1945, 1969, 1989, 2001,
  ];
  const preserved = currentPuzzleYears.every((year) => CURATED_HISTORICAL_YEARS.includes(year));
  if (!preserved) {
    logger.error("Not all current puzzle years are preserved");
    return false;
  }
