...
describe('Enhanced Proximity Feedback', () => {
  describe('getEnhancedProximityFeedback', () => {
    ...
    it('should provide context-aware encouragement for century-level distances', () => {
      ...
      expect(feedback.encouragement.toLowerCase()).toMatch(/nice try|good guess|not bad/);
    });

    it('should handle perfect guesses with celebration', () => {
      const feedback = getEnhancedProximityFeedback(1969, 1969);
      
      expect(feedback.message.toLowerCase()).toMatch(/correct|perfect|excellent/);
      expect(feedback.encouragement.toLowerCase()).toMatch(/amazing|incredible|outstanding/);
    });
  ...
  describe('getProgressiveFeedback', () => {
    it('should acknowledge improvement when getting closer', () => {
      ...
      expect(feedback.improvementMessage?.toLowerCase()).toMatch(/warmer|closer|better|improving/);
    });
    ...
    it('should provide encouragement for first guess', () => {
      ...
      expect(feedback.improvementMessage?.toLowerCase()).toMatch(/good start|first guess|solid attempt/);
    });
    ...
    it('should acknowledge when moving away from target', () => {
      ...
      expect(feedback.improvementMessage?.toLowerCase()).toMatch(/further|colder|back up/);
    });
    ...
    it('should handle consistent distance attempts', () => {
      ...
      expect(feedback.improvementMessage?.toLowerCase()).toMatch(/same distance|try different/);
    });
  });
  ...
  describe('getHistoricalContextHint', () => {
    ...
    it('should adjust hint specificity based on distance', () => {
      ...
      if (closeHint.hint && farHint.hint) {
        expect(closeHint.hint.length).toBeGreaterThanOrEqual(farHint.hint.length - 50);
      }
    });
